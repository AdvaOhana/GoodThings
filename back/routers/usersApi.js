...
usersApiRouter.get('/logout',(req, res) => { 
    //Nerya, handle logout and remove session from user using MID. then send the response if good from here, if not from the logout MID 
        res.status(400).json({message:`Logged Out id:${req.sId} !`})
})
